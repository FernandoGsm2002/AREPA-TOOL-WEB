...
import { Activity, AlertTriangle, Box, Users, KeyRound, RefreshCw, LogOut, Search, ShieldAlert, ShieldCheck, Upload } from "lucide-react";
...
export default function Admin() {
  ...
  return <div className="bg-background min-h-screen lg:flex">
    ...
    <main className="min-w-0 flex-1 p-5 sm:p-9"><header className="mb-7 flex items-start justify-between gap-4"><div><p className="text-primary text-xs font-bold tracking-[.14em] uppercase">Control operativo</p><h1 className="font-display mt-1 text-3xl font-bold tracking-tight">{tabs.find(x=>x.id===tab)?.label}</h1></div><Button variant="outline" size="sm" onClick={()=>loadTab()}><RefreshCw className="size-4"/>Actualizar</Button></header>{notice&&<p className="border-destructive/25 bg-destructive/8 text-destructive mb-5 rounded-lg border px-3 py-2 text-sm">{notice}</p>}
      ...
      {tab==="resellers" && <section className="grid gap-3 md:grid-cols-2">{resellers.map(r=><article key={r.id} className="border-border/60 bg-card rounded-xl border p-5"><div className="flex justify-between gap-3"><div><p className="font-semibold">{r.name}</p><p className="text-muted-foreground text-xs">{r.username} · {r.status}</p></div><p className="font-mono text-lg">{money(r.balance)}</p></div><p className="text-muted-foreground mt-4 text-xs">3m {money(r.service_price_3m)} · 6m {money(r.service_price_6m)} · 12m {money(r.service_price)}</p><Button size="sm" className="mt-4" onClick={()=>credit(r.id)}>Recargar saldo</Button></article>)}</section>}
